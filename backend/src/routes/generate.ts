...
- Не используй роботские фразы: "повысить эффективность", "эффективные решения", "оптимизировать процессы", "инновационные решения", "давайте обсудим сотрудничество".
...
- Максимум 3-4 коротких предложения.
- 1 предложение: конкретное наблюдение про ${target}; начни с "Смотрел...", "Заметил..." или "Есть ощущение...".
...
- Используй осторожные формулировки, если уместно: "скорее всего", "возможно".
...
Хорошо:
Заметил, что у SaaS-страницы продукт становится понятен не сразу. Люди, скорее всего, уходят раньше. Я делаю лендинги, где ценность видно быстрее. Показать пару примеров?
  ...
  if (language === "russian") {
    if (previousMessage || followUpType) {
      return tone === "professional"
        ? `Заметил еще одну вещь: для ${target} возможно проще проверить ${offer} на одном маленьком участке, без больших изменений. Так быстрее понятно, есть ли смысл двигаться дальше. Открыты к короткому взгляду?`
        : `Заметил еще одну вещь: для ${target} ${offer} возможно проще проверить на одном маленьком участке. Так быстрее понятно, есть ли смысл. Показать, как это может выглядеть?`;
    }

    return tone === "professional"
      ? `Смотрел ${target}, и есть ощущение, что смысл оффера теряется в первых строках. Люди, скорее всего, уходят раньше. ${offer} помогает сделать это понятнее без лишнего шума. Открыты к короткому взгляду?`
      : `Смотрел ${target}, и есть ощущение, что смысл оффера теряется в первых строках. Люди, скорее всего, уходят раньше. ${offer} помогает сделать это понятнее без лишнего шума. Показать пару примеров?`;
  }
